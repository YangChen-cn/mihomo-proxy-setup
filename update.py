...
proxy-groups:
  - name: "🚀 节点选择"
    type: select
    proxies: ["♻️ 自动选择", {yaml_names}, "DIRECT"]
  - name: "♻️ 自动选择"
    type: url-test
...
rules:
  ...
  # Google (incl. Antigravity / Code Assist / Gemini): reachable directly
  # from this VM (HK), so DIRECT is faster and immune to proxy node rescans.
  # NOTE: hostnames like daily-cloudcode-pa.googleapis.com are NOT subdomains
  # of cloudcode-pa.googleapis.com (label boundary!), so match googleapis.com.
  ...
  - GEOIP,CN,DIRECT
  # non-OpenAI traffic rides the auto group; it may drift to a bad node
  # without touching Codex, and node rescans bounce this group only
  - MATCH,♻️ 自动选择
"""
    ...
    try:
        ...
        # world-readable: no secrets in the config, lets scan_nodes.py read it
...
# user-writable copy takes priority so rescans don't need root
USER_HK_FILE = Path("/home/yang/mihomo-setup/hk-endpoints.txt")
...
def load_hk_endpoints() -> set[str]:
    ...
    The user-writable copy wins so rescans don't require root.
    """
    for f in (USER_HK_FILE, HK_ENDPOINTS_FILE):
        if f.exists():
            eps = {ln.strip() for ln in f.read_text().splitlines() if ln.strip()}
            if eps:
                return eps
    return set()
